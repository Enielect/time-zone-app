...
import React, {type CSSProperties, type RefObject, useRef, useState } from 'react';
      ...
      <AnimatePresence mode="wait">
        {!open ? (
          <motion.button
            ...
            className={cn(
              'border border-black/10 hover:border-black/20 bg-white px-4 py-2.5 rounded-xl',
              buttonClassName
            )}
          >
            <motion.p layout layoutId="form-label" className="flex items-center text-black gap-2 text-sm">
              {icon} {label}
            </motion.p>
          </motion.button>
        ) : (
          <motion.div
            ...
            style={{"--blur-sm": '7px'} as CSSProperties}
            key="form-container"
            className={cn(
              'shadow-md rounded-2xl flex flex-col min-w-[400px] backdrop-blur-sm overflow-hidden relative',
              containerClassName
            )}
          >
            <div className="p-4">
